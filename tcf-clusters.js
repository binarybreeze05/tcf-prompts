/* TCF prompts — sort dropdown, done-ticks, hide-done, similarity / coverage
   / difficulty / practice modes. Adapted from tef-clusters.js; the 10-cluster
   feature (TEF-only) is dropped. */
(function(){
  function init(){
    ...
    numbered.forEach(function(it){
      ...
      cb.setAttribute("aria-label", "Marquer " + it.num + " comme fait");
      var txt = document.createElement("span");
      txt.textContent = "Fait";
    ...
    // prepared response, task format aside) carries "Aussi au TEF · <section>" —
    ...
    (function(){
      ...
      if (!X) return;
      var fr = true;
      var SEC = { A:["Oral A","tef-section-a-practice-order"], B:["Oral B","tef-section-b-practice-order"], EA:["Écrit A","ecrite-section-a-practice-order"], EB:["Écrit B","ecrite-section-b-practice-order"] };
      ...
      numbered.forEach(function(it){
        ...
        refs.slice(0, 3).forEach(function(r){
          ...
          a.textContent = (r.t === "c" ? "Aussi au TEF · " : "≈ TEF · ") + sec[0];
          a.title = "TEF " + sec[0] + " · n° " + r.n.slice(0, 12).join(", ") + (r.n.length > 12 ? ", … (" + r.n.length + " au total)" : "") + (r.t === "c" ? " — même réponse à préparer" : " — proche, réponse à adapter");
          headAdd(head, a);
        });
      });
    })();
    ...
    function refreshDone(){
      ...
      if (!ndone){
        ui.progText.textContent = "Aucun fait pour l'instant · " + total + " " + unit + " à faire";
      } else if (!left){
        ui.progText.textContent = "Tous les " + total + " sont faits.";
      } else {
    ...
    ui.progReset.addEventListener("click", function(){
      ...
      if (!n) return;
      if (!window.confirm("Effacer les " + n + " coches « Fait » pour cette liste ?")) return;
    ...
    function loadCoverage(done){
      loadData("tcf-coverage.js", "TCF_COVERAGE", "Chargement des données de couverture…", function(){
        if (!window.TCF_COVERAGE) state.coverageFailed = true;
        done();
      });
    }
    function loadSimilarity(done){
      loadData("tcf-similarity.js", "TCF_SIMILARITY", "Chargement des données de similarité…", function(){
    ...
    loadSimilarity(function(){
      if (!window.TCF_SIMILARITY || !window.TCF_SIMILARITY[setKey]){
        ...
        render();
        ui.desc.textContent = "L'ordre de similarité n'a pas pu être chargé — ordre pratique affiché.";
        return;
      }
      render();
    });
    ...
    function paint(){
      ...
      ui.summary.textContent = "Ordre pratique (original) · " + sections.length + " " + unit;
      ui.desc.textContent = "";
    }
    ...
    function renderCoverage(){
      ...
      if (!pk){
        ui.desc.textContent = state.coverageFailed
          ? "Impossible de charger tcf-coverage.js — les autres ordres restent disponibles."
          : "Pas de données de couverture pour cette liste.";
        ui.summary.textContent = "Ordre de couverture indisponible"; return;
      }
      clearCoverage();
      ...
      if (pk.caps && pk.caps.length){
        ...
        ch.textContent = "Avant tout — ces règles fixent un plafond quel que soit votre niveau";
        caps.appendChild(ch);
      ...
      pk.order.forEach(function(o){
        ...
        if (head){
          ...
          chip.querySelector("i").textContent = "sur " + total;
          headAdd(head, chip);
        }
        ...
        note.innerHTML =
          ...
          +   '<span class="tcf-cov-m"><em>Pool couvert</em>'
          +     '<span class="tcf-cov-bar"><i></i></span><b></b></span>'
          +   '<span class="tcf-cov-m"><em>Machine langue</em>'
        ...
        if (ms){
          ...
          band.querySelector(".tcf-ms-kicker span").textContent =
            "Après " + o.rank + " sujet" + (o.rank === 1 ? "" : "s") + " · "
            + (o.cum_pool || 0) + "% du pool couvert";
      ...
      ui.summary.textContent = "Ordre de couverture · " + total + " " + unit;
    }
    ...
    function renderSimilarity(){
      ...
      if (!pk){
        ui.desc.textContent = state.similarityFailed
          ? "Impossible de charger tcf-similarity.js — les autres ordres restent disponibles."
          : "Pas de données de similarité pour cette liste.";
        ui.summary.textContent = "Ordre de similarité indisponible"; return;
      }
      clearSimilarity();
      ...
      ui.desc.textContent = pk.blurb || "";
      ui.summary.textContent = "Ordre de similarité · " + total + " " + unit;
    }
    ...
    function buildToolbar(){
      ...
      optSim.textContent = "Similarité (défaut · chaque sujet près de son plus proche voisin)";
      sel.appendChild(optSim);
      var optCov = document.createElement("option");
      optCov.value = "coverage";
      optCov.textContent = "Couverture (le plus fort d'abord, avec pourquoi)";
      sel.appendChild(optCov);
      var optD = document.createElement("option");
      optD.value = "default";
      optD.textContent = "Ordre pratique (l'ordre du fichier)";
      sel.appendChild(optD);
      if (hasDifficulty){
        var optDiff = document.createElement("option");
        optDiff.value = "difficulty"; optDiff.textContent = "Difficulté (rang #1 = plus facile)";
      ...
      var hideTx = document.createElement("span"); hideTx.textContent = "Cacher les faits";
